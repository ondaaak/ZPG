...
void BezierTransform::setControlPoints(const std::vector<glm::vec3>& points) {
...
void BezierTransform::update(float deltaTime) {
    ...
    if (t > (float)numSegments) {
        t = fmod(t, (float)numSegments);
    }
}
...
glm::mat4 BezierTransform::getMatrix() const {
    ...
    glm::vec3 tangent = 3.0f * oneMinusT * oneMinusT * (p1 - p0) +
...
Transformation* BezierTransform::clone() const {
    return new BezierTransform(controlPoints, speed, true);
}